...
class PasswordResetForm(forms.Form):
    ...
    def get_users(self, email: str) -> Iterator[User]:
        active_user_qs = User.objects.filter(email__iexact=email, is_active=True)
        return (
            user
    ...
    def make_uidb64_and_token(self, email: str) -> Iterator[Tuple[str, str]]:
        for user in self.get_users(email):
            user_pk_base64 = urlsafe_base64_encode(force_bytes(user.pk))
            token = token_generator.make_token(user)
            yield (user_pk_base64, token)
